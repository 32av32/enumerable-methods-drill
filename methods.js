...
function removeAddresses(array) {
    // Глубокое копирование в целях сохранности оригинального массива
...
function getUsersGeo(array, id) {
    console.log(array.find(item => item.id === id).address.geo)
}
...
function getUsersCompany(array, id) {
    console.log(array.find(item => item.id === id).company)
}

getUsersCompany(users, 10)
...
function changePhone(array, id, phone) {
    return array.map(item => {
        if (item.id === id) item.phone = phone
        return item
    })
}

console.log(changePhone(users, 10, '+79280010101'))